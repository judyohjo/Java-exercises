public class Book {
	...
	public boolean removeAuthorByName(String by_name) {
		boolean bool = false;
		for(int i=0; i<authors.size(); i++) {
			Author author = authors.get(i);
			for(int j=0; j<authors.size(); j++) {
				if(author.getName() == by_name) {
	...
	public boolean removeAuthorByEmail(String by_email) {
		boolean bool = false;
		for(int i=0; i<authors.size(); i++) {
			Author author = authors.get(i);
			for(int j=0; j<authors.size(); j++) {
				if(author.getEmail() == by_email) {
					authors.remove(author);
					bool = true;
				}
			}
